...
function Product({ id, title, price, description, category, image, rating, fluid, home, special }) {
    const router = useRouter();

    return (
        <>
            {
                !fluid ? <div className={`product_card md:shadow-md bg-white p-1 md:p-4 inline-block md:flex flex-col items-start gap-2 overflow-hidden ${special && "mr-1"}`}>
                    <h2 onClick={() => router.push(`/products/${id}`)} className="font-medium md:font-bold text-base md:text-[22px] capitalize">
                        {
                            title.length > 25 ? <>{title.slice(0, 12)}...</> : <>{title}</>
                        }
                    </h2>
